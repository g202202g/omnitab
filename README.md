...
当你在任意网页上时，可以直接把页面里的某个区域做成新标签页里的「网页卡片」：

- 在目标网页上点击扩展图标（浏览器工具栏里的「万象标签 OmniTab」）
- 页面会进入“选取模式”：悬停高亮，单击选中要显示的区域
- 选中后会自动打开一个“真正的新标签页”（地址栏仍是 `chrome://newtab`），并弹出「添加卡片」窗口（已预填链接与选区），确认后才会创建

小技巧：

- 按 `Esc` 可随时取消
- 按住 `Shift` 再点击，可以更容易选到“父级容器”（适合页面 DOM 很深、选区太碎的情况）
- 选中后仍可在卡片设置里继续调整「只显示其中一块」的选择器
